...
const API_PRODUCTS_URL = environment.ApiRoot + '/receipts';
...
export class receiptsService {
	...
	// READ
	getAllItems(): Observable<receiptsModel[]> {

		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<receiptsModel[]>(API_PRODUCTS_URL + '?more=true', { headers: httpHeaders });
	}
	...
	Createreceipts(item): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.post<any>(API_PRODUCTS_URL, item, { headers: httpHeaders });
	}

	// UPDATE => PUT: update the product on the server
	Updatereceipts(item: receiptsModel): Observable<any> {
